...
void autocomplete(struct TrieNode *root, const char *prefix, char suggestions[][MAX_WORD_LENGTH], int *count) {
    ...
    find_all_suggestions(node, new_prefix, strlen(new_prefix),suggestions, count);
}
...
int main() {
    ...
    // Load words and phrases from a file
    const char *filename = "words.txt"; // Update the filename to include both words and phrases
    ...
    do{
        ...
        printf("---------------------------------------------------------------\n");
        char choice[10];
        printf("Enter 1 if you want to search:\n");
